...
static void OnMessageArrived(MessageData *data)
{
    ...
    // 取出消息内容
    // 如果格式化后的字符串长度大于等于size，超过size的部分会被截断，只将其中的(size-1)个字符复制到str中，
    // 并给其后添加一个字符串结束符'\0'。所以需要+1。
    char message[512] = {0};
    snprintf(message, (int)data->message->payloadlen + 1,
             "%s", (char *)data->message->payload);

    // 显示消息，用"|"分割换行（目前的oled驱动不支持自动换行，接收到的消息也不包含'\r'或'\n'）
    char *p = strtok(message, "|");
    while (p != NULL)
    {
        ssd1306_PrintString(p, Font_7x10, White);       // 显示一行
        ssd1306_PrintString("\r\n", Font_7x10, White);  // 回车换行
        p = strtok(NULL, "|");                          // 分割出下一行
    }
}
