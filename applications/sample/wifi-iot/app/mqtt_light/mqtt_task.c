...
static void OnMessageArrived(MessageData *data)
{
    ...
    // 分解出命令
    // 命令格式："color,state"
    // color指定灯的颜色。1：红色，2：绿色，3：黄色
    // state指定灯的状态。1：关闭，2：打开
    int color = 0;
    int state = 0;

    // 按逗号分割出color
    char *p = strtok((char *)data->message->payload, ",");
    if (p)
        color = atoi(p); // 转换为数字，转换失败返回0
    
    // 按逗号分割出state
    p = strtok(NULL, ",");
    if (p)
        state = atoi(p); // 转换为数字，转换失败返回0
