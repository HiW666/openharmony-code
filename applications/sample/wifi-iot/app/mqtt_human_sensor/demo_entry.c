...
#include "human_sensor.h"   // 炫彩灯板人体红外传感器接口
...
static void mqttDemoTask(void *arg)
{
    ...
    // 向主题"OpenHarmony/devices/Pegasus0001/HumanSensor/Message"发布上线消息
    char *ptopic = "OpenHarmony/devices/Pegasus0001/HumanSensor/Message";       // 主题
    ...
    // 向主题"OpenHarmony/devices/Pegasus0001/HumanSensor/Message"上报人体红外传感器数据
    // 工作循环
    while (1)
    {
        char payload[64] = {0};                                         // 消息内容
        snprintf(payload, sizeof(payload),
                 "{\"device_id\":\"Pegasus0001\", \"human\":\"%d\"}",
                 GetHuman());                                           // 获取人体红外传感器数据
        int rc = MqttTaskPublish(ptopic, payload);                      // 发布消息
        if (rc != 0)                                                    // 检查是否成功发布消息
            printf("MQTT Publish failed!\r\n");
        else
